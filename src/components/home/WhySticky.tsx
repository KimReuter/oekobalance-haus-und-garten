...
const PANELS = [
  { title: "Baggerarbeiten",    img: "/bagger-2.jpg",    text: "Vom Aushub bis zum Fundament – die Basis für jedes Projekt." },
  { title: "Naturstein & Wege", img: "/gartenweg-2.jpg", text: "Haltbare Mauern und Wege, die Gelände in Form bringen." },
  { title: "Terrassen & Gärten",img: "/gartenbau-2.jpg", text: "Individuelle Außenbereiche, die Haus & Natur verbinden." },
  { title: "Innenausbau",       img: "/tiny-2.jpg",      text: "Dachboden, Wintergarten oder Raumteiler – mit Liebe zum Detail." },
];
...
export default function WhySticky() {
  ...
  const eX = useTransform(scrollYProgress, [eStart, (eStart + eEnd)/2, OUT_AT, 1], ["-48px", "0px", "0px", "-48px"]);
  const eO = useTransform(scrollYProgress, [eStart, eEnd, OUT_AT, 1], [0, 1, 1, 0]);

  const hX = useTransform(scrollYProgress, [hStart, (hStart + hEnd)/2, OUT_AT, 1], ["48px", "0px", "0px", "48px"]);
  const hO = useTransform(scrollYProgress, [hStart, hEnd, OUT_AT, 1], [0, 1, 1, 0]);

  const sX = useTransform(scrollYProgress, [sStart, (sStart + sEnd)/2, OUT_AT, 1], ["-48px", "0px", "0px", "-48px"]);
  const sO = useTransform(scrollYProgress, [sStart, sEnd, OUT_AT, 1], [0, 1, 1, 0]);

  return (
    <section ref={ref} className={SECTION_CLASS}>
      <div className="sticky top-0 h-screen grid grid-cols-1 md:grid-cols-2 items-center">
        {/* LEFT: texts reintroduced with scroll-binding */}
        <div className="px-8">
          <motion.p
        ...
        {/* RIGHT: fullscreen panels + moving caption (mode switchable) */}
        <div className="relative h-screen w-full overflow-hidden">
          {PANELS.map((p, i) => {
            ...
            const scale   = useTransform(scrollYProgress, [start, start + PAD, end - PAD, end], [1.015, 1.015, 1.06, 1.06]);
            ...
            const xTravel = LINE_MODE === "horizontal"
              ? useTransform(local, [0, 1], ["120%", "-120%"])
              : useTransform(local, [0, 1], ["0%", "0%"]); // fixed in x for vertical

            const yTravel = LINE_MODE === "horizontal"
              ? useTransform(local, [0, 1], ["0%", "0%"])  // fixed in y for horizontal
              : useTransform(local, [0, 1], ["120%", "-120%"]);

            // slight fade at segment edges
            const lineOpacity = useTransform(local, [0.0, 0.05, 0.95, 1.0], [0, 1, 1, 0]);

            return (
              <motion.div key={i} className="absolute inset-0 h-full w-full" style={{ opacity }}>
                ...
                {LINE_MODE === "horizontal" ? (
                  <motion.div
                  aria-hidden
                  className={`absolute inset-x-0 ${H_POS === "center" ? "top-1/2 -translate-y-1/2" : "bottom-0"} 
                ...
                >
                  ...
                  >
                    ...
                    <span className="font-normal opacity-90">{p.text}</span>
                  </motion.div>
                </motion.div>
                ) : (
                  <motion.div
...
>
  ...
  >
    ...
    <span className="font-light opacity-90">{p.text}</span>
  </motion.div>
</motion.div>
                )}
              </motion.div>
